...
public class PacificAtlanticWaterFlow {
	...
	private Deque<int[]> initPacQ(int[][] matrix) {
	...
	private Deque<int[]> initAtlQ(int[][] matrix) {
	...
	private void bfs(int[][] matrix, Deque<int[]> Q, boolean[][] boolArr, boolean[][] visited) {
		while (!Q.isEmpty()) {
			int[] point = Q.poll();
			int row = point[0];
			int col = point[1];
	...
	private List<List<Integer>> buildResult(boolean[][] pacBool, boolean[][] atlBool) {
